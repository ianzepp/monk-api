...
CREATE INDEX "idx_tenant_name_active" ON "tenants" ("name", "is_active");
CREATE INDEX "idx_tenant_database" ON "tenants" ("database");
CREATE INDEX "idx_tenant_trashed" ON "tenants" ("trashed_at") WHERE "trashed_at" IS NOT NULL;
CREATE INDEX "idx_tenant_deleted" ON "tenants" ("deleted_at") WHERE "deleted_at" IS NOT NULL;
...
COMMENT ON COLUMN "tenants"."database" IS 'PostgreSQL database name containing tenant data (direct tenant name)';
