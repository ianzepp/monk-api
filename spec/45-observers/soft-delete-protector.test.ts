    ...
    describe('Trashed Record Detection', () => {
        ...
        test('should block operations on trashed records', async () => {
            ...
            try {
                ...
                expect((error as SecurityError).message).toContain('Cannot update trashed records: record-2');
                expect((error as SecurityError).code).toBe('SOFT_DELETE_PROTECTION');
            }
        });
        ...
        test('should block operations on multiple trashed records', async () => {
            ...
            try {
                ...
                expect((error as SecurityError).message).toContain('Cannot delete trashed records: record-1, record-3');
            }
        });
    });
    ...
    describe('Hard Deleted Record Detection', () => {
        test('should block operations on deleted records', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(SecurityError);
                expect((error as SecurityError).message).toContain('Cannot update permanently deleted records: record-2');
                expect((error as SecurityError).code).toBe('HARD_DELETE_PROTECTION');
            }
        });
        ...
        test('should block operations on multiple deleted records', async () => {
            ...
            try {
                ...
                expect((error as SecurityError).message).toContain('Cannot delete permanently deleted records: record-1, record-2');
            }
        });
        ...
        test('should prioritize trashed over deleted in error message', async () => {
            ...
            try {
                ...
                expect((error as SecurityError).message).toContain('trashed records: record-1');
                expect((error as SecurityError).code).toBe('SOFT_DELETE_PROTECTION');
            }
        });
    });
