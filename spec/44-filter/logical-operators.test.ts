...
describe('Logical Operators - Comprehensive Testing', () => {
    describe('$and operator', () => {
        ...
        test('AND with mixed operator types', () => {
            const { whereClause, params } = FilterWhere.generate({
                $and: [{ name: { $like: 'John%' } }, { age: { $between: [18, 65] } }, { tags: { $any: ['vip', 'premium'] } }, { status: { $nin: ['banned', 'suspended'] } }],
            });
    ...
    describe('$or operator', () => {
        ...
        test('OR with different data types', () => {
            const { whereClause, params } = FilterWhere.generate({
                $or: [{ age: null }, { age: { $gte: 21 } }, { status: { $in: ['verified', 'premium'] } }],
            });
    ...
    describe('$not operator', () => {
        ...
        test('NOT with array operations', () => {
            const { whereClause, params } = FilterWhere.generate({
                $not: [{ access_deny: { $any: ['user-123', 'group-456'] } }],
            });
    ...
    describe('$nor operator', () => {
        ...
        test('NOR with array operations', () => {
            const { whereClause, params } = FilterWhere.generate({
                $nor: [{ access_deny: { $any: ['user-123'] } }, { blacklist: { $all: ['restricted'] } }],
            });
    ...
    describe('Complex Logical Combinations', () => {
                            ...
                            { role: 'admin' },
                            {
                                $and: [
                                    ...
                                    { verified: true },
                                    {
                                        $not: [
                                            {
                                                $or: [{ status: 'suspended' }, { access_deny: { $any: ['user-123'] } }],
                                            },
                                        ],
                                    },
                                ],
        ...
        test('mixed logical operators', () => {
            const { whereClause, params } = FilterWhere.generate({
                $and: [
                    {
                        $nand: [{ role: 'guest' }, { verified: false }],
                    },
                    {
        ...
        test('parameter indexing with complex nesting', () => {
            const { whereClause, params } = FilterWhere.generate({
                $or: [
                    ...
                    {
                        $and: [{ field4: 'value4' }, { field5: 'value5' }],
                    },
                ],
            });
    ...
    describe('Performance Edge Cases', () => {
        ...
        test('deeply nested with many branches', () => {
            const { whereClause, params } = FilterWhere.generate({
                $or: [
                    {
                        $and: [{ type: 'user' }, { $or: [{ role: 'admin' }, { role: 'mod' }] }],
                    },
                    {
                        $and: [{ type: 'system' }, { $or: [{ role: 'service' }, { role: 'api' }] }],
                    },
                    {
                        $and: [{ type: 'guest' }, { verified: true }],
                    },
                ],
            });
    ...
    describe('Real-World ACL Scenarios', () => {
        test('user access with role fallback', () => {
            ...
            const { whereClause, params } = FilterWhere.generate({
                $or: [{ access_read: { $any: [userId, ...userGroups] } }, { access_edit: { $any: [userId, ...userGroups] } }, { access_full: { $any: [userId, ...userGroups] } }],
            });
            ...
            expect(params).toEqual([userId, ...userGroups, userId, ...userGroups, userId, ...userGroups]);
        });
        ...
        test('access with explicit denial override', () => {
            const { whereClause, params } = FilterWhere.generate({
                $and: [
                    {
                        $or: [{ access_read: { $any: ['user-123'] } }, { access_edit: { $any: ['user-123'] } }],
                    },
                    {
                        $not: [{ access_deny: { $any: ['user-123'] } }],
                    },
                ],
            });
