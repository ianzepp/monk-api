    ...
    describe('Wildcard Detection', () => {
        test('should detect wildcard patterns', () => {
            ...
            paths.forEach(path => {
                ...
                const hasWildcards = parts.some(part => part.includes('*') || part.includes('?'));
        ...
        test('should extract wildcard components', () => {
            ...
            const parts = path.split('/').filter(p => p.length > 0);
            const wildcards = parts.filter(part => part.includes('*') || part.includes('?'));
...
//# sourceMappingURL=ftp-path-parsing.test.js.map