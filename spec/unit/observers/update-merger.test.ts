...
describe('Unit: UpdateMerger Observer', () => {
    ...
    describe('Data Validation', () => {
        ...
        test('should skip when data is not an array', async () => {
            mockContext.data = { id: 'not-array' } as any;
    ...
    describe('Preload Error Handling', () => {
        test('should throw error when preload failed', async () => {
            ...
            try {
                ...
                expect((error as BusinessLogicError).message).toContain('Cannot merge update data');
                expect((error as BusinessLogicError).code).toBe('UPDATE_MERGE_FAILED');
            }
        });
    });
    ...
    describe('Record Merging', () => {
        test('should merge existing record with update data', async () => {
            ...
            expect(new Date(mockContext.data[0].updated_at).getTime()).toBeGreaterThan(
