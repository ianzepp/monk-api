...
describe('JsonSchemaValidator', () => {
    ...
    describe('validation success', () => {
        test('should validate all records successfully', async () => {
            ...
            // Should log success
            expect(mockContext.system.info).toHaveBeenCalledWith('JSON Schema validation completed', expect.objectContaining({
                schemaName: 'test_schema',
                operation: 'create',
                recordCount: 1,
                validatedCount: 1,
    ...
    describe('operation handling', () => {
        test('should handle create operations', async () => {
            ...
            mockContext.schema.validateOrThrow.mockImplementation(() => { });
            await validator.execute(mockContext);
            expect(mockContext.system.info).toHaveBeenCalledWith('JSON Schema validation completed', expect.objectContaining({ operation: 'create' }));
        });
        test('should handle update operations', async () => {
            mockContext.operation = 'update';
            mockContext.schema.validateOrThrow.mockImplementation(() => { });
            await validator.execute(mockContext);
            expect(mockContext.system.info).toHaveBeenCalledWith('JSON Schema validation completed', expect.objectContaining({ operation: 'update' }));
        });
    });
    describe('edge cases', () => {
