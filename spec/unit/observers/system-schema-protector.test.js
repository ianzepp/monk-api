...
describe('SystemSchemaProtector', () => {
    ...
    describe('system schema protection', () => {
        test('should allow operations on user schemas', async () => {
            ...
            // Should log success
            expect(mockContext.system.info).toHaveBeenCalledWith('System schema protection check passed', expect.objectContaining({
                schemaName: 'user_schema',
                operation: 'create',
    ...
    describe('operation handling', () => {
        test('should handle create operations', async () => {
            ...
            mockContext.schema.isSystemSchema.mockReturnValue(false);
            await protector.execute(mockContext);
            expect(mockContext.system.info).toHaveBeenCalledWith('System schema protection check passed', expect.objectContaining({ operation: 'create' }));
        });
        test('should handle update operations', async () => {
            mockContext.operation = 'update';
            mockContext.schema.isSystemSchema.mockReturnValue(false);
            await protector.execute(mockContext);
            expect(mockContext.system.info).toHaveBeenCalledWith('System schema protection check passed', expect.objectContaining({ operation: 'update' }));
        });
        test('should handle delete operations', async () => {
            mockContext.operation = 'delete';
            mockContext.schema.isSystemSchema.mockReturnValue(false);
            await protector.execute(mockContext);
            expect(mockContext.system.info).toHaveBeenCalledWith('System schema protection check passed', expect.objectContaining({ operation: 'delete' }));
        });
    });
...
//# sourceMappingURL=system-schema-protector.test.js.map