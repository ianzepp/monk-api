    ...
    describe('Preload Error Handling', () => {
        test('should throw error when preload failed', async () => {
            ...
            try {
                ...
                expect(error.message).toContain('Cannot validate record existence');
                expect(error.code).toBe('EXISTENCE_VALIDATION_FAILED');
            }
        });
    });
    describe('Existence Validation', () => {
        ...
        test('should throw error when some records are missing', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(BusinessLogicError);
                expect(error.message).toContain('Cannot update - Record not found: record-2');
        ...
        test('should throw error when multiple records are missing', async () => {
            ...
            try {
                ...
                expect(error.message).toContain('Cannot delete - Records not found: record-2, record-3');
            }
        });
        test('should handle single missing record with singular message', async () => {
            ...
            try {
                ...
                expect(error.message).toContain('Record not found: missing-record');
            }
        });
    });
    describe('Revert Operation Special Validation', () => {
        ...
        test('should fail revert when records are not trashed', async () => {
            ...
            try {
                ...
                expect(error).toBeInstanceOf(BusinessLogicError);
                expect(error.message).toContain('Cannot revert non-trashed records: record-1, record-2');
                expect(error.code).toBe('CANNOT_REVERT_NON_TRASHED');
            }
        });
        test('should fail revert when some records are not trashed', async () => {
            ...
            try {
                ...
                expect(error.message).toContain('Cannot revert non-trashed records: record-2');
            }
        });
    });
...
//# sourceMappingURL=existence-validator.test.js.map