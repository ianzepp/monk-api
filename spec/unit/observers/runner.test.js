...
import { createMockSystem, createMockObserver, createValidationObserver, assertObserverExecuted, assertErrorAdded, assertWarningAdded } from '../../helpers/observer-helpers.js';
// Mock the ObserverLoader
vi.mock('@src/lib/observers/loader.js');
const mockObserverLoader = ObserverLoader;
describe('ObserverRunner', () => {
    ...
    describe('execute', () => {
        test('should execute rings 0-9 in order', async () => {
            ...
            const result = await runner.execute(mockSystem, 'create', 'users', { name: 'test' });
        ...
        test('should stop execution on validation errors before database ring', async () => {
            const validationObserver = createValidationObserver(ObserverRing.Validation, true, // add error
            false, // no warning
            'ErrorValidator');
            const auditObserver = createMockObserver(ObserverRing.Audit, undefined, 'audit');
            mockObserverLoader.getObservers.mockImplementation((schema, ring) => {
        ...
        test('should continue execution on warnings', async () => {
            const validationObserver = createValidationObserver(ObserverRing.Validation, false, // no error
            true, // add warning
            'WarningValidator');
            const auditObserver = createMockObserver(ObserverRing.Audit, undefined, 'audit');
            mockObserverLoader.getObservers.mockImplementation((schema, ring) => {
        ...
        test('should handle observer execution failures gracefully', async () => {
            const throwingObserver = createMockObserver(ObserverRing.Validation, undefined, 'ThrowingObserver', true // should throw
            );
            mockObserverLoader.getObservers.mockImplementation((schema, ring) => {
                if (ring === ObserverRing.Validation)
        ...
        test('should respect operation filtering', async () => {
            const createOnlyObserver = createMockObserver(ObserverRing.Validation, ['create'], // only for create operations
            'CreateOnlyObserver');
            ...
            const createResult = await runner.execute(mockSystem, 'create', 'users', { name: 'test' });
            ...
            const updateResult = await runner.execute(mockSystem, 'update', 'users', { name: 'test' }, 'user-id');
            expect(updateResult.success).toBe(true);
            expect(createOnlyObserver.execute).not.toHaveBeenCalled();
        });
        test('should provide complete context to observers', async () => {
            const contextCheckObserver = createMockObserver(ObserverRing.Validation, undefined, 'ContextChecker');
            ...
            await runner.execute(mockSystem, 'update', 'users', { name: 'updated' }, 'user-123', { name: 'original' });
