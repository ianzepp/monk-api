...
describe('FTP Utility Functions - Unit Tests', () => {
  ...
  describe('Content Type Detection', () => {
    test('should detect email field content type', () => {
      ...
      if (fieldName?.includes('email')) contentType = 'text/plain';
      if (fieldName?.includes('url') || fieldName?.includes('link')) contentType = 'text/uri-list';
      if (fieldName?.includes('html')) contentType = 'text/html';
  ...
  describe('Error Handling Patterns', () => {
    ...
    test('should classify error types', () => {
      ...
      errorTests.forEach(({ error, expectedCode }) => {
        ...
        if (error.includes('not found')) code = 'NOT_FOUND';
        else if (error.includes('schema')) code = 'SCHEMA_ERROR';
        else if (error.includes('validation') || error.includes('invalid')) code = 'VALIDATION_ERROR';
        else if (error.includes('database')) code = 'DATABASE_ERROR';
