...
import type { 
    ...
    ProcessedContent,
    ValidationResult
} from '@src/routes/ftp/store.js';
import type {
    FtpDeleteRequest,
    FtpDeleteResponse, 
    FtpDeletePath,
    DeleteValidationResult
} from '@src/routes/ftp/delete.js';
...
describe('FTP File Operations - Unit Tests (Phase 3)', () => {
    ...
    describe('FTP Store Path Parser', () => {
        test('should parse record storage paths', () => {
            ...
            const parsePath = (path: string): FtpStorePath => {
    ...
    describe('FTP Content Processor', () => {
        ...
        test('should process record content', () => {
            const processContent = (content: any, path: FtpStorePath, options: any): ProcessedContent => {
        ...
        test('should process field content', () => {
            const processContent = (content: any, path: FtpStorePath): ProcessedContent => {
                return {
                    processed_content: content,
                    content_type: 'text/plain',
                    size: Buffer.byteLength(String(content), 'utf8'),
    ...
    describe('FTP Permission Validator', () => {
        test('should allow root user all operations', () => {
            const validatePermission = (isRoot: boolean): ValidationResult => {
                if (isRoot) {
        ...
        test('should validate record permissions', () => {
            const validateRecordPermission = (
                user: any, 
                record: any, 
                ...
                const hasEditPermission = record.access_edit?.some((id: string) => userContext.includes(id));
        ...
        test('should validate field permissions', () => {
            const validateFieldPermission = (user: any, record: any, fieldName: string): ValidationResult => {
                if (!record) {
                    return {
                        allowed: false,
    ...
    describe('FTP Delete Path Parser', () => {
        test('should parse delete paths correctly', () => {
            const parsePath = (path: string): FtpDeletePath => {
                const cleanPath = path.replace(/\/+/g, '/').replace(/\/$/, '') || '/';
                const parts = cleanPath.split('/').filter(p => p.length > 0);
    ...
    describe('FTP Delete Permission Validator', () => {
        test('should require force flag for dangerous operations', () => {
            const validateDangerousOperation = (
                isDangerous: boolean, 
                force: boolean
            ): DeleteValidationResult => {
        ...
        test('should validate delete permissions for records', () => {
            const validateDeletePermission = (user: any, record: any): DeleteValidationResult => {
                if (!record) {
                    return {
                        allowed: false,
                        reason: 'record_not_found',
                ...
                const userContext = [user.id, ...(user.accessRead || [])];
                const hasFullPermission = record.access_full?.some((id: string) => userContext.includes(id));
    ...
    describe('FTP Transaction Management', () => {
        test('should generate valid transaction IDs', () => {
            const generateTransactionId = (operation: string): string => {
        ...
        test('should manage transaction state', () => {
            ...
            const beginTransaction = (operation: string): string => {
            ...
            const commitTransaction = (id: string): boolean => {
            ...
            const rollbackTransaction = (id: string): boolean => {
                const tx = transactions.get(id);
                if (tx && tx.status === 'active') {
        ...
        test('should handle transaction timeouts', () => {
            const createTransactionWithTimeout = (timeoutMs: number) => {
    ...
    describe('FTP Store Request Validation', () => {
        test('should validate store request structure', () => {
            const validateStoreRequest = (request: Partial<FtpStoreRequest>): boolean => {
                return !!(
                    request.path &&
            ...
            const validRequest: FtpStoreRequest = {
        ...
        test('should apply default options', () => {
            const applyDefaults = (options: Partial<FtpStoreRequest['ftp_options']>) => {
                return {
                    binary_mode: false,
                    overwrite: true,
                    append_mode: false,
    ...
    describe('FTP Delete Request Validation', () => {
        test('should validate delete request structure', () => {
            const validateDeleteRequest = (request: Partial<FtpDeleteRequest>): boolean => {
                return !!(
                    request.path &&
                    request.ftp_options
                );
            ...
            const validRequest: FtpDeleteRequest = {
        ...
        test('should apply safety-first defaults', () => {
            const applyDeleteDefaults = (options: Partial<FtpDeleteRequest['ftp_options']>) => {
                return {
                    recursive: false,
                    force: false,
                    permanent: false,
