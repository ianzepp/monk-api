...
if ACCOUNT_RESULT=$(cat "$(dirname "$0")/../../test/schemas/account.yaml" | monk meta create schema 2>&1); then
...
if CONTACT_RESULT=$(cat "$(dirname "$0")/../../test/schemas/contact.yaml" | monk meta create schema 2>&1); then
    # Validate YAML response
