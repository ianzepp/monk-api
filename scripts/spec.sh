...
if [[ -z "$pattern_or_path" ]]; then
    print_header "Running complete test suite (TypeScript → Shell)"
else
    print_header "Running tests for: $pattern_or_path (TypeScript → Shell)"
fi
...
if [[ $overall_exit_code -eq 0 ]]; then
    ...
    echo -e "${GREEN}✅ Both TypeScript and Shell tests successful${NC}"
else
