 ...
 * Reduces translation overhead for repeated FTP operations and provides
...
export class PatternCache {
    ...
    private static enforceCapacityLimits(): void {
        ...
        const entries = Array.from(this.cache.entries()).sort((a, b) => 
            a[1].last_used.getTime() - b[1].last_used.getTime()
        );
    ...
    private static scheduleCleanupIfNeeded(): void {
        const now = new Date();
        const nextCleanup = new Date(this.stats.last_cleanup.getTime() + 
    ...
    private static estimateTranslationCost(complexity: 'simple' | 'complex' | 'cross'): number {
        switch (complexity) {
            case 'simple': return 5;   // 5ms
            case 'complex': return 20; // 20ms
            case 'cross': return 50;   // 50ms
            default: return 10;
        }
    }
}