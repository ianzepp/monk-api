...
import type { Pipeline, PipelineContext } from '@src/lib/pipeline/interfaces.js';
import type { PipelineRing, OperationType } from '@src/lib/pipeline/types.js';
import {
    ValidationError,
    BusinessLogicError,
    SystemError,
 ...
 */
export abstract class BaseObserver implements Pipeline {
    abstract readonly ring: PipelineRing;
     ...
     *
     * This method should be called by the PipelineRunner. It wraps the
     * execute() method with consistent error handling, logging, and execution profiling.
     */
    async executeTry(context: PipelineContext): Promise<void> {
        ...
        try {
            ...
            logger.time(`Pipeline: ${observerName}`, startTime, {
                ring: this.ring,
                operation,
                schemaName,
                status: 'success'
            ...
            this.handlePipelineError(error, observerName, context, 0); // Duration not needed since timing logged above
        }
    }
    ...
    async executeOne(record: any, context: PipelineContext): Promise<void> {
        ...
        // Pipelines can implement this for simple per-record processing
    }
    ...
    private createTimeoutPromise(observerName: string): Promise<never> {
        return new Promise((_, reject) => {
            setTimeout(() => {
                reject(new PipelineTimeoutError(observerName, this.timeoutMs));
            }, this.timeoutMs);
        });
    }
    ...
    private handlePipelineError(
        ...
        if (error instanceof ValidationError) {
        ...
        } else if (error instanceof SystemError || error instanceof PipelineTimeoutError) {
            // Unrecoverable system errors - should rollback entire transaction
            logger.warn('Pipeline system error', {
                observerName,
            ...
            logger.warn('Pipeline unknown error', {
                observerName,
                operation: context.operation,
                schemaName: context.schema.name,
                error: error.message,
            ...
            context.warnings.push(warning);
            logger.warn('Pipeline unknown error (non-Error object)', {
                observerName,
                operation: context.operation,
                schemaName: context.schema.name,
                error: String(error),
     ...
     */
    protected needsTransaction(context: PipelineContext, reason?: string): void {
     ...
     */
    protected static getTransactionReasons(context: PipelineContext): Array<{
        observer: string;
        ring: number;
        reason: string;
        timestamp: number;
     ...
     */
    protected static isTransactionRequired(context: PipelineContext): boolean {
