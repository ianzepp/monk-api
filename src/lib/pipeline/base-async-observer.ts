 ...
 */
export abstract class BaseAsyncPipeline implements Pipeline {
    ...
    async executeTry(context: PipelineContext): Promise<void> {
        ...
        setImmediate(async () => {
            try {
                ...
                logger.time(`AsyncPipeline: ${observerName}`, startTime, {
                    ring: this.ring,
                    operation,
                    schemaName,
                    status: 'success'
     ...
     */
    abstract execute(context: PipelineContext): Promise<void>;
