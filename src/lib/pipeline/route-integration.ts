/**
 * Pipeline Route Integration Helpers
 *
...
import type { OperationType, PipelineResult } from '@src/lib/pipeline/types.js';
 ...
 */
export async function executePipelinePipeline(
    system: System,
    ...
    // since it's not going through Database.runPipelinePipeline()
 ...
 */
export async function executePipelinePipelineBatch(
    system: System,
    operation: OperationType,
    schema: string,
    records: any[]
    ...
    for (const [index, record] of records.entries()) {
        try {
            const result = await executePipelinePipeline(
                system,
                operation,
                schema,
                record,
 ...
 */
export function handlePipelineResult(
    context: Context,
    ...
    if (!result.success) {
        const errorMessage = result.errors.length > 0
            ...
            : 'Pipeline pipeline validation failed';
 ...
 */
export function handleBatchPipelineResult(
    context: Context,
