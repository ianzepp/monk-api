...
import { handleInput, printPrompt, writeToStream } from './session-handler.js';
 ...
 */
export function createSSHServer(config?: TTYConfig): { stop: () => void } {
                ...
                client.on('session', (accept) => {
                    ...
                    sshSession.on('exec', async (accept, _reject, info) => {
                        ...
                        try {
                            ...
                            writeToStream(stream, `Error: ${err instanceof Error ? err.message : String(err)}\n`);
                        }
