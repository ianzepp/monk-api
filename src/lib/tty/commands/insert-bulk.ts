 ...
 *
 * Usage:
 *   insert_bulk <path> < data.json
 *   insert_bulk <path> --file=data.json
 *   cat data.json | insert_bulk <path>
 *
 ...
 *
 * Examples:
 *   cat users.json | insert_bulk /api/data/users
 *   insert_bulk /api/data/products --file=products.json
 *   insert_bulk /api/data/orders --dry-run < orders.json
 */
...
export const insert_bulk: CommandHandler = async (session, fs, args, io) => {
    if (args.length === 0) {
        io.stderr.write('insert_bulk: missing path\n');
        io.stderr.write('Usage: insert_bulk <path> [--file=<file>] [--dry-run] < data.json\n');
        return 1;
    }
    ...
    for (const arg of args) {
        if (arg.startsWith('--file=')) {
            ...
            if (pathArg === null) {
                ...
                io.stderr.write('Note: insert_bulk does not accept key=value pairs. Use insert for single records.\n');
                return 1;
            }
        } else {
    ...
    // Extract model name from path (e.g., /api/data/users -> users)
    const pathParts = resolved.split('/').filter(p => p);
    if (pathParts.length < 3 || pathParts[0] !== 'api' || pathParts[1] !== 'data') {
        io.stderr.write(`insert_bulk: path must be /api/data/<model>\n`);
        return 1;
    }
    ...
    if (dryRun) {
        io.stdout.write(`Dry run: would insert ${records.length} records into ${modelName}\n`);
        return 0;
    }
    ...
    try {
        ...
        const response = await app.fetch(new Request(`http://localhost/api/data/${modelName}`, {
            method: 'POST',
            headers: {
        ...
        io.stdout.write(`Inserted ${count} records into ${modelName}\n`);
        return 0;
