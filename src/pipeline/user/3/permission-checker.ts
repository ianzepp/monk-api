...
import { ValidationError, BusinessLogicError } from '@src/lib/pipeline/errors.js';
...
export default class PermissionChecker extends BaseObserver {
    readonly ring = PipelineRing.Business;
    ...
    private async validateUserCreation(context: PipelineContext, currentUserLevel: number): Promise<void> {
        const { data } = context;
    ...
    private async validateUserUpdate(context: PipelineContext, currentUserLevel: number): Promise<void> {
    ...
    private async validateOtherPermissions(context: PipelineContext, currentUserLevel: number): Promise<void> {
        const { data, existing } = context;
