...
import { ValidationError, BusinessLogicError, ValidationWarning } from '@src/lib/pipeline/errors.js';
...
export default class BalanceValidator extends BaseObserver {
    ...
    private async validateNewAccount(context: PipelineContext): Promise<void> {
        const { data } = context;
    ...
    private async validateBalanceUpdate(context: PipelineContext): Promise<void> {
