...
export default async function (context: Context): Promise<any> {
    try {
                ...
                if (await TenantService.databaseExists(tenant.database)) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            const schemaResult = await client.query("SELECT COUNT(*) as count FROM information_schema.tables WHERE table_name = 'schemas'");
        ...
        const httpStatus = healthCheck.status === 'healthy' ? 200 : healthCheck.status === 'warning' ? 200 : 503;
