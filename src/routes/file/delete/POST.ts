...
import { FileTransactionManager, type FileTransaction } from '@src/routes/file/store/POST.js';

// Enhanced File Delete Transport Types (Phase 3)
...
// Enhanced File Delete Response (Phase 3)
...
/**
 * Enhanced File Delete Path Parser (Phase 3)
 */
class FileDeletePathParser {
    static parse(path: string): FileDeletePath {
        ...
        if (parts[0] !== 'data' && parts[0] !== 'meta') {
            throw new Error('File delete only supports /data and /meta paths');
        }
        ...
        throw new Error(`Invalid File delete path format: ${path} - too many path components`);
    }
...
/**
 * File Delete Permission Validator (Phase 3)
 */
class FileDeletePermissionValidator {
    static async validateDeletePermission(system: any, path: FileDeletePath, options: FileDeleteRequest['file_options']): Promise<DeleteValidationResult> {
...
class FileDeleteOperations {
    static async executeRecordDelete(system: any, path: FileDeletePath, options: FileDeleteRequest['file_options']): Promise<DeleteOperationResult> {
    ...
    static async executeFieldDelete(system: any, path: FileDeletePath, options: FileDeleteRequest['file_options']): Promise<DeleteOperationResult> {
        const recordId = path.record_id!;
    ...
    static async executeSchemaDelete(system: any, path: FileDeletePath, options: FileDeleteRequest['file_options']): Promise<DeleteOperationResult> {
...
/**
 * POST /api/file/delete - Enhanced File Deletion Middleware (Phase 3)
 *
 ...
 */
export default async function fileDeleteHandler(context: Context): Promise<any> {
    const system = context.get('system');
    const requestBody: FileDeleteRequest = await context.req.json();
    ...
    logger.info('File delete operation (Phase 3)', {
    ...
    try {
        ...
        const options = {
            ...
            recursive: requestBody.file_options.recursive ?? false,
            force: requestBody.file_options.force ?? false,
            permanent: requestBody.file_options.permanent ?? false,
            atomic: requestBody.file_options.atomic ?? true,
        };

        // Parse File path to understand the delete operation
        const filePath = FileDeletePathParser.parse(requestBody.path);

        // Safety check for dangerous operations
        if (filePath.is_dangerous && !options.force) {
        ...
        if (options.atomic && !requestBody.metadata?.transaction_id) {
            transactionId = await FileTransactionManager.beginTransaction(system, 'delete', requestBody.path);
        } else if (requestBody.metadata?.transaction_id) {
            transactionId = requestBody.metadata.transaction_id;
        }

        // Validate permissions
        const permissionCheck = await FileDeletePermissionValidator.validateDeletePermission(system, filePath, options);
        ...
        switch (filePath.operation_type) {
            case 'record':
                operationResult = await FileDeleteOperations.executeRecordDelete(system, filePath, options);
                break;

            case 'field':
                operationResult = await FileDeleteOperations.executeFieldDelete(system, filePath, options);
                break;

            case 'schema':
                operationResult = await FileDeleteOperations.executeSchemaDelete(system, filePath, options);
                break;

            default:
                throw new Error(`Unsupported delete operation type: ${filePath.operation_type}`);
        }
        ...
        logger.info('File delete completed (Phase 3)', {
            path: requestBody.path,
            operation: operationResult.operation,
