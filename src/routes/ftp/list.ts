...
export interface FtpListResponse {
    ...
    total: number;
    has_more: boolean;
    pattern_info?: {                     // Pattern analysis metadata
...
class FtpPathParser {
    ...
    private static estimateMatches(translation: WildcardTranslation): number {
        ...
        switch (translation.complexity) {
            case 'simple': return Math.max(1, 100 - translation.estimated_cost);
            case 'complex': return Math.max(1, 50 - Math.floor(translation.estimated_cost / 2));
            case 'cross': return Math.max(1, 200 - translation.estimated_cost);
            default: return 10;
        }
    }
}
    ...
    try {
        ...
        switch (ftpPath.type) {
            ...
            case 'schema':
                ...
                // Add sorting
                filterData.order = options.sort_by === 'date' 
                    ? `updated_at ${options.sort_order || 'desc'}`
                    : `id ${options.sort_order || 'asc'}`;
        ...
        const response: FtpListResponse = {
            ...
            pattern_info: {
                ...
                pattern_breakdown: ftpPath.wildcards.length > 0 ? {
