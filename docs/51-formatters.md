...
curl http://localhost:9001/api/auth/whoami?pick=data.id
→ c81d0a9b-8d9a-4daf-9f45-08eb8bc3805c

# Extract multiple fields (returns JSON object)
curl http://localhost:9001/api/auth/whoami?pick=data.id,data.name
...
curl http://localhost:9001/api/auth/whoami?pick=data.id,data.name&format=toon
→ id: c81d0a9b...
  name: Demo User
```
...
```bash
curl http://localhost:9001/auth/login?pick=data.token \
...
**With Field Extraction:**
```bash
curl http://localhost:9001/api/auth/whoami?pick=data.id,data.access&format=toon
→ id: c81d0a9b...
  access: root
```
...
**With Field Extraction:**
```bash
curl http://localhost:9001/api/auth/whoami?pick=data.access_read,data.access_edit&format=yaml
→ access_read: []
  access_edit: []
```
...
curl http://localhost:9001/auth/tenants?pick=data[0].name&format=brainfuck > tenant.bf
```
...
curl http://localhost:9001/api/auth/whoami?pick=data.name&format=morse
```
...
curl http://localhost:9001/auth/login?pick=data.token&format=qr \
  -d '{"tenant":"demo","username":"root"}'
```
...
**Single Field:**
```bash
?pick=data.id                    # Returns plain text value
?pick=data.user.email           # Nested path support
```

**Multiple Fields:**
```bash
?pick=data.id,data.name         # Returns JSON object with both fields
?pick=data.id,data.access       # Comma-separated paths
```
...
# Extract and format
curl /api/auth/whoami | jq -r '.data.id' | some-formatter
```

**After (with ?pick):**
```bash
# Extract single field (returns plain text)
curl /api/auth/whoami?pick=data.id
→ c81d0a9b-8d9a-4daf-9f45-08eb8bc3805c

# Extract multiple fields (returns JSON object)
curl /api/auth/whoami?pick=data.id,data.name
→ {"id":"c81d0a9b...","name":"Demo User"}

# Extract and format (one request)
curl /api/auth/whoami?pick=data.id,data.name&format=toon
→ id: c81d0a9b...
  name: Demo User
```
...
- **Nested Paths**: Use dot notation (`.`) to traverse objects: `data.user.email`
- **Multiple Fields**: Comma-separate paths to extract multiple fields
...
TOKEN=$(curl /auth/login?pick=data.token -d '{"tenant":"demo","username":"root"}')

# Verify specific field value
USER_ACCESS=$(curl /api/auth/whoami?pick=data.access)
[[ "$USER_ACCESS" == "root" ]] && echo "Admin access confirmed"
```

**CI/CD Pipelines:**
```bash
# Extract schema count for validation
SCHEMA_COUNT=$(curl /api/describe?pick=data | jq 'length')

# Get database name for backup scripts
DB_NAME=$(curl /api/auth/whoami?pick=data.database)
...
curl /api/data/users/123?pick=data.email

# Compare fields across formats
curl /api/auth/whoami?pick=data.id,data.name&format=toon
curl /api/auth/whoami?pick=data.id,data.name&format=yaml
```
...
3. **Field Extraction**: Extracts specified fields if `?pick=` present
...
// - ?pick=data.id → extracts field first
// - ?pick=data.id&format=toon → extracts then encodes
```
...
- Field extraction only runs when `?pick=` is present
...
- Integration tests verify `?pick=` works with all formats
...
- Format compatibility (`?pick=` + `?format=`)
...
# With field extraction
monk curl GET '/api/auth/whoami?pick=data.id'

# With format
monk curl GET '/api/auth/whoami?format=toon'

# Combined
monk curl GET '/api/auth/whoami?pick=data.id,data.name&format=yaml'
```
...
curl /auth/tenants?pick=data[0].name
→ demo-01

# Get all tenant names (if data is array of objects)
curl /auth/tenants?pick=data&format=json | jq -r '.[].name'
```

### Chain with jq (When Needed)
```bash
# Extract then transform with jq
curl /api/auth/whoami?pick=data | jq -r '.access'

# Or just extract directly
curl /api/auth/whoami?pick=data.access
```
...
curl /api/auth/whoami?pick=data.id,data.name&format=toon
curl /api/auth/whoami?pick=data.id,data.name&format=yaml
curl /api/auth/whoami?pick=data.id,data.name&format=markdown
```
...
curl /api/auth/whoami?pick=data.id,data.access&format=toon
→ id: c81d0a9b...
  access: root
```
...
### Missing Fields
```bash
curl /api/auth/whoami?pick=data.nonexistent
→ null  (graceful)
```

### Invalid Path Syntax
```bash
curl /api/auth/whoami?pick=data..invalid
→ null  (graceful - treats as missing)
```

### Empty Pick Parameter
```bash
curl /api/auth/whoami?pick=
...
#!/bin/bash
TOKEN=$(curl /auth/login?pick=data.token -d '{"tenant":"demo","username":"root"}')
USER_ID=$(curl /api/auth/whoami?pick=data.id -H "Authorization: Bearer $TOKEN")
```
...
✅ **Field Extraction**: Server-side `?pick=` eliminates `| jq` piping
