...
export class ContactGenerator extends BaseGenerator {
    ...
    contactTypes = ['customer', 'vendor', 'partner', 'employee', 'lead', 'prospect'];
    statuses = ['active', 'inactive', 'pending', 'qualified'];
    priorities = ['low', 'normal', 'high', 'urgent'];
    sources = ['website', 'referral', 'advertising', 'trade_show', 'cold_call', 'other'];
    tags = [
        'vip', 'technical', 'decision-maker', 'budget-holder', 'influencer',
        'champion', 'blocker', 'early-adopter', 'key-contact', 'stakeholder'
    ];
    generate(count, options, context) {
        const contacts = [];
     ...
     */
    generateContactType(index) {
        if (index % 2 === 0)
            return 'customer'; // 50% customers
        if (index % 4 === 1)
            return 'prospect'; // 25% prospects
        if (index % 8 === 3)
            return 'partner'; // 12.5% partners
        if (index % 16 === 7)
            return 'employee'; // 6.25% employees
        if (index % 32 === 15)
            return 'lead'; // 3.125% leads
    ...
    generateStatus(index) {
        if (index % 5 === 0)
            return 'inactive'; // 20% inactive
        if (index % 7 === 1)
            return 'pending'; // ~14% pending
        if (index % 6 === 2)
            return 'qualified'; // ~17% qualified
        return 'active'; // ~49% active
    }
    /**
     * Generate priority with realistic distribution
     */
    generatePriority(index) {
        if (index % 10 === 0)
            return 'urgent'; // 10% urgent
        if (index % 5 === 1)
            return 'high'; // 20% high
        if (index % 8 === 2)
            return 'low'; // 12.5% low
     ...
     */
    generateJobTitle(index, options) {
     ...
     */
    generateSource(index) {
        // 10% have no source
        if (index % 10 === 9) {
            return null;
        }
        if (index % 3 === 0)
            return 'website'; // 30% website
        if (index % 5 === 1)
            return 'referral'; // 20% referral
        if (index % 7 === 2)
            return 'advertising'; // ~14% advertising
        if (index % 11 === 3)
            return 'trade_show'; // ~9% trade show
        if (index % 13 === 4)
            return 'cold_call'; // ~8% cold call
        return 'other'; // Remaining other
    }
    /**
     * Generate account relationship (nullable, UUID)
     */
    generateAccountId(context, index, options) {
     ...
     */
    generateNotes(firstName, lastName, index, options) {
     ...
     */
    generateLastContactedDate(index) {
     ...
     */
    generateEdgeCases(context, options) {
        const edgeCases = [
            {
...
//# sourceMappingURL=contact-generator.js.map