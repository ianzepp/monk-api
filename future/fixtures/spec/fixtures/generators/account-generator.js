...
export class AccountGenerator extends BaseGenerator {
    ...
    accountTypes = ['personal', 'business', 'trial', 'premium'];
     ...
     */
    generateAccountType(index) {
        if (index % 10 === 0)
            return 'business'; // 10% business
        if (index % 20 === 1)
            return 'premium'; // 5% premium
        if (index % 15 === 2)
            return 'trial'; // ~7% trial
        return 'personal'; // ~78% personal
    }
    /**
     * Generate balance (0 to 1,000,000 per schema)
     */
    generateBalance(index) {
     ...
     */
    generateLastLogin(createdAt, index) {
        // 20% never logged in
        if (index % 5 === 0) {
            return null;
        }
