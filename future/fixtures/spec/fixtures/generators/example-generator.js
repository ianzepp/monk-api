...
export class ExampleGenerator extends BaseGenerator {
    ...
    categories = ['technology', 'business', 'education', 'health', 'entertainment', 'other'];
    tags = [
     ...
     */
    generateDescription(index, options) {
    ...
    generateStatus(index) {
        if (index % 2 === 0)
            return 'approved'; // 50% approved
        if (index % 5 === 1)
            return 'pending'; // 20% pending
        if (index % 7 === 2)
            return 'draft'; // ~14% draft
        if (index % 11 === 3)
            return 'rejected'; // ~9% rejected
    ...
    generatePriority(index) {
        if (index % 10 === 0)
            return 5; // 10% highest priority
        if (index % 5 === 1)
            return 4; // 20% high priority
        if (index % 7 === 2)
            return 2; // ~14% low priority
        if (index % 9 === 3)
            return 1; // ~11% lowest priority
     ...
     */
    generateCategory(index) {
        if (index % 3 === 0)
            return 'technology'; // 33% technology
        if (index % 4 === 1)
            return 'business'; // 25% business
        if (index % 5 === 2)
            return 'education'; // 20% education
        if (index % 7 === 3)
            return 'health'; // ~14% health
        if (index % 11 === 4)
            return 'entertainment'; // ~9% entertainment
     ...
     */
    generateUpdatedDate(createdAt, index) {
     ...
     */
    generateExpiresAt(createdAt, index) {
        // 70% don't expire
        if (index % 10 < 7) {
            return null;
        }
     ...
     */
    generateVisibility(index) {
        if (index % 2 === 0)
            return 'public'; // 50% public
        if (index % 5 === 1)
            return 'private'; // 20% private
        return 'restricted'; // 30% restricted
    }
    /**
     * Generate max revisions (1-100)
     */
    generateMaxRevisions(index) {
        if (index % 10 === 0)
            return 100; // 10% unlimited (max)
        if (index % 5 === 1)
            return 50; // 20% high limit
        if (index % 3 === 2)
            return 5; // ~33% low limit
