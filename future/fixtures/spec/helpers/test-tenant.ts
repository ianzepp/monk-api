 ...
 */
export async function createTestContextWithFixture(fixtureName: string, options: TemplateLoadOptions = {}): Promise<TestContextWithData> {
    ...
    if (customFixture) {
        ...
        fixture = {
            ...
            schemas: customFixture.schemas.reduce(
                (acc, schema) => {
 ...
 */
export async function createMultiFixtureContext(fixtureNames: string[], options: TemplateLoadOptions = {}): Promise<TestContextWithData> {
 ...
 */
async function createMockData(context: TestContext, fixture: any, customData?: Record<string, any[]>): Promise<Record<string, number>> {
 ...
 */
async function createManualData(context: TestContext, fixture: any, customData?: Record<string, any[]>): Promise<Record<string, number>> {
...
export function validateFixtureComposition(fixtureNames: string[]): {
    ...
    Object.entries(problematicCombinations).forEach(([fixture, conflicts]) => {
        if (fixtureNames.includes(fixture)) {
            ...
            if (presentConflicts.length > 0) {
                warnings.push(`Fixture '${fixture}' includes data from ${presentConflicts.join(', ')}. ` + `Consider using only '${fixture}' to avoid duplicate data.`);
            }
        }
    });
    ...
    if (fixtureNames.length > 4) {
        warnings.push(`Composing ${fixtureNames.length} fixtures may result in very large datasets. ` + `Consider using fewer, more targeted fixtures.`);
    }
 ...
 */
async function createCustomFixtureData(context: TestContext, customFixture: CustomFixtureDefinition): Promise<Record<string, number>> {
 ...
 */
export async function createTestContextWithCustomFixture(customFixture: CustomFixtureDefinition, options: Omit<TemplateLoadOptions, 'customFixture'> = {}): Promise<TestContextWithData> {
