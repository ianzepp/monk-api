 ...
 */
export async function migrateTestToFixture(oldTestSetup: () => Promise<TestContext>, fixtureName: string, options: TemplateLoadOptions = {}): Promise<TestContextWithData> {
...
function analyzeTestComplexity(testCode: string): TestAnalysis {
    ...
    const setupOperations = [/createTestTenant/g, /createTestContext/g, /readFile.*\.json/g, /metabase\.createOne/g, /database\.createOne/g];
...
async function wrapLegacyContext(oldContext: TestContext, fixtureName: string): Promise<TestContextWithData> {
    ...
    const helpers = {
        ...
        hasSchema(schemaName: string): boolean {
            ...
            return ['accounts', 'contacts', 'user', 'schemas'].includes(schemaName);
        },
