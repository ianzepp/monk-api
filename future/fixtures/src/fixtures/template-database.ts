...
export class TemplateDatabase {
    ...
    private static createDatabaseClient(databaseName: string): pg.Client {
        return DatabaseConnection.getClient(databaseName);
    }
    /**
    ...
    static async listTemplates(): Promise<string[]> {
        ...
        try {
            await client.connect();
            const result = await client.query(`SELECT datname FROM pg_database WHERE datname LIKE '${this.TEMPLATE_PREFIX}%' ORDER BY datname`);
    ...
    static async cleanTemplates(pattern?: string): Promise<void> {
        ...
        const templatesToClean = pattern ? templates.filter(name => name.includes(pattern)) : templates;

        if (templatesToClean.length === 0) {
            logger.info(pattern ? `No templates found matching pattern: ${pattern}` : 'No templates found to clean');
            return;
        }
